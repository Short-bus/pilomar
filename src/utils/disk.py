...
class discmonitor:  # 2 references.
    ...
    def __init__(
        ...
        self.Path = (
            path  # This is the path without any device label (ie /media/pi  or   /   )
        )
    ...
    def FindUSB(self, devname="/dev/sda1"):
        ...
        if result:  # Previous steps succeeded.
            ...
            if (
                ...
                textline = (
                    "discmonitor.FindUSB: Media label '"
                    + self.USBLabel
        ...
        if result:  # OK so far.
            ...
            if (
                ...
                lines = [
                    ...
                    "There may be a false folder named "
                    + self.DfPath
                    + " on the SD card which prevents mounting the USB memory.",
                    "To solve :-",
        ...
        for line in lines:
            ...
            if dev in self.USBScanList:  # This is a potential device...
                ...
                if label in self.USBAlarmLabels:  # Reject this type of device.
                    ...
                    print(
                        "discmonitor.SelectUSBdevname: Found banned USB device connected. (",
                        label,
                        ")",
                    )
    ...
    def ListUSBdevices(self):
        ...
        for line in lines:
            ...
            if self.Log != None:
                ...
                print("discmonitor.ListUSBDevices: Considering", line)
            items = self.SplitSpaces(
                line
            )  # Separate by spaces, but ignore spaces in quotes.
            dev = items[0].split(":")[0]  # Remove the trailing ':' from the device.
            if dev in self.USBScanList:  # This is a potential device...
                if self.Log != None:
                    ...
                    print("discmonitor.ListUSBDevices: Including", line)
                label = ""  # No label yet.
                for item in items:  # Find the label entry.
                    if item.startswith("LABEL="):
                        label = item.split("=")[1].replace(
