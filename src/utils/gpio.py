...
class inputpin_gpio:
    ...
    def Status(self):
        ...
        temp = (
            ...
            + " IsOn:"
            + str(self.IsOn()).rjust(5)
            + " IsOff:"
            + str(self.IsOff()).rjust(5)
            + " IsHigh:"
            + str(self.IsHigh()).rjust(5)
            + " IsLow:"
            + str(self.IsLow()).rjust(5)
        )
        return temp
...
class outputpin_gpiod:
    ...
    def ReleaseAll():
        ...
        for pin in outputpin_gpiod.OutputPins:
            try:
                ...
                MainLog.Log(
                    "outputpin_gpiod.ReleaseAll(): Failed to release pin:",
                    e,
                    level="error",
                )
    ...
    def IsHigh(self):
        ...
        Does not respect self.Invert flag.
        To respect self.Invert, use IsOn()/IsOff()"""
        if self.Line.get_value(self.Pin) != 0:
            result = True  # The true electrical state of the pin.
