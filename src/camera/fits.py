...
# Image dimensions
width = 4056
if "--width" in ArgumentDict:
    width = int(ArgumentDict["--width"][0])
height = 3040
if "--height" in ArgumentDict:
    height = int(ArgumentDict["--height"][0])

# Image quality (for jpg)
quality = 100
if "--quality" in ArgumentDict:
    quality = int(ArgumentDict["--quality"][0])

# Sensor data format.
sensor_format = "SBGGR12"
if "--format" in ArgumentDict:
    sensor_format = ArgumentDict["--format"][0]
...
if "--awbgains" in ArgumentDict:
    csl = ArgumentDict["--awbgains"]["all"].split(
        ","
