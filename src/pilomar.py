...
ProgramTitle = (
    SourceCode().split("/")[-1].split(".")[0].lower()
...
from pilomarcamera import (
    ...
    astrolens,
    astrocamera,
)  # Pilomar's CAMERA elements.
print("Got through camera")
from skyfield.api import Star, Topos, EarthSatellite
from skyfield.api import (
    Loader,
)  # Create own 'load' functionality by specifying the download directory this way.
from skyfield.api import load_constellation_names
from skyfield.magnitudelib import (
    planetary_magnitude,
)  # Calculate the realtime apparent magnitude of solar system objects.
...
def RelativeAltAz(StarAlt, StarAz, LookAtAlt, LookAtAz):
    ...
    """
    PlotX, PlotY, PlotZ = AltAzToXYZ(
        StarAlt, StarAz
    )  # Place star on celestial sphere (unit 1)

    # Swing round to LOOK-AT Azimuth.
    NewY = PlotY * math.cos(math.radians(-1 * LookAtAz)) - PlotX * math.sin(
        math.radians(-1 * LookAtAz)
    )  # 0degrees is due north on Y axis. 90degrees is due east on X axis.
    NewX = PlotX * math.cos(math.radians(-1 * LookAtAz)) + PlotY * math.sin(
        math.radians(-1 * LookAtAz)
    )
    PlotX = NewX
    PlotY = NewY
    ...
    NewY = PlotY * math.cos(math.radians(-1 * LookAtAlt)) - PlotZ * math.sin(
        math.radians(-1 * LookAtAlt)
    )  # 0degrees is due north on Y axis. 90degrees is straight up on Z axis.
    NewZ = PlotZ * math.cos(math.radians(-1 * LookAtAlt)) + PlotY * math.sin(
        math.radians(-1 * LookAtAlt)
    )
    PlotY = NewY
    PlotZ = NewZ

    PlotStarAlt, PlotStarAz = XYZToAltAz(
        PlotX, PlotY, PlotZ
...
ObservationStatusWindow.SetBorderColors(
...
ObservationStatusWindow.PlaceString(
    "           Target: [TARGET                                                           ]",
    row=1,
    col=0,
)
...
for i in [
    ...
    if not SessionWindow.InitializeColorRange(
        i,
        badfg=OSW_TEXT_BAD,
        badbg=OSW_TEXT_BG,
...
class sessionstatus(attributemaster):
    ...
    def ShowRemoteStatus(self):
        ...
        if Mctl.DeviceFailure:
            SessionWindow.FieldColor("DF", fg=OSW_TEXT_BAD)
        else:
            SessionWindow.FieldColor("DF", fg=OSW_TEXT_GOOD)
        SessionWindow.FieldValue(
        ...
        if self.AutonomousControl:
            SessionWindow.FieldColor("AC", fg=OSW_TEXT_GOOD)
        else:
            SessionWindow.FieldColor("AC", fg=OSW_TEXT_POOR)
        SessionWindow.FieldValue(
            "RCL", self.RemoteControl
        )  # Flag to show microcontroller allows remote control.
        if self.RemoteControl:
            SessionWindow.FieldColor("RCL", fg=OSW_TEXT_GOOD)
        else:
            SessionWindow.FieldColor("RCL", fg=OSW_TEXT_BAD)
        SessionWindow.FieldValue(
            "CS", self.ClockSynchronised
        )  # Flag to show that microcontroller flag is synchronised.
        if self.ClockSynchronised:
            SessionWindow.FieldColor("CS", fg=OSW_TEXT_GOOD)
        else:
            SessionWindow.FieldColor("CS", fg=OSW_TEXT_BAD)
        SessionWindow.FieldValue(
        ...
        if self.TrajectorySafetyFlushes > 0:
            SessionWindow.FieldColor("MTSF", fg=OSW_TEXT_BAD)
        else:
            SessionWindow.FieldColor("MTSF", fg=OSW_TEXT_GOOD)
        if (
            Mctl.PoweredByUsb
        ...
        for i in MotorControls:  # Report trajectory information for each motor.
            ...
            if i.MotorConfigured:
                SessionWindow.FieldColor(fnp + "C", fg=OSW_TEXT_GOOD)
            else:
                SessionWindow.FieldColor(fnp + "C", fg=OSW_TEXT_POOR)
            SessionWindow.FieldValue(
            ...
            if i.TrajectoryEntries > 0:
                SessionWindow.FieldColor(fnp + "D", fg=OSW_TEXT_GOOD)
            else:
                SessionWindow.FieldColor(fnp + "D", fg=OSW_TEXT_POOR)
            if self.MotorControlMode == "trajectory":
                SessionWindow.FieldValue(fnp + "T", i.OnTarget)
                if i.OnTarget:
                    SessionWindow.FieldColor(fnp + "T", fg=OSW_TEXT_GOOD)
                else:
                    SessionWindow.FieldColor(fnp + "T", fg=OSW_TEXT_POOR)
...
def ChooseLastTarget():
    ...
    if se.SearchGroup == "solar":
        ...
        MainLog.Log(
            "ChooseLastTarget: Unrecognised searchgroup: Line " + str(temp),
            level="error",
        )
    return obstarget
...
def MarkupPreview(drift_pixels_x=None, drift_pixels_y=None, astrotime=None):
    ...
    if True:  # Mark Right Ascension direction on the image.
        ...
        PlotAlt1, PlotAz1 = RelativeAltAz(
    ...
    if (
        ...
        for entryfrom, entryto, entryname in ConstellationLinks:
            ...
            if (
                ...
                PlotFromAlt, PlotFromAz = RelativeAltAz(
                ...
                PlotToAlt, PlotToAz = RelativeAltAz(
...
def ObservationRun():
    ...
    temp = (
        colordisplay.GlobalWindowLimits()
